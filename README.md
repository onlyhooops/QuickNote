...
- **AI 助手（可选）**：设置页启用并填入 DeepSeek Key 后，编辑器可一键「✨ AI 探索」——对<strong>纯文字</strong>做出处研判与内容补全；审核后以「✨ AI 补全」区块插入、与原文排版区分（自动剔除图片/网页卡/音乐/视频卡片，不参与分析）
...
- **规划：BOB 右键插件快速笔记** —— 接入 BOB（https://bobtranslate.com/plugin/ ）生态，实现「在任意应用选中文本 → 呼出 BOB → 一键把原文/译文快速记入快记」：
  - BOB 插件侧：新增动作按钮「记入快记」，将选中文本（可含原文+译文）发往本地服务；
  - 快记侧：新增极简接收接口 `POST /api/quickin`（文本/可选标签/来源），校验仅本机来源（预留访问令牌），立即生成一个时间点并轻提示；
  - 依赖既有"纯文本录入 + 标签"能力，不触碰图片/卡片。
- 从备份恢复入口
- 远端网页图片抓取到本地
